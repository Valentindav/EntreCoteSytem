...
void Inputs::Update()
{
    ...
    for (int i = 0; i < Keyboard::AMOUNT; i++)
    {
        if (s_keyboardStates[i] == DOWN)
            s_keyboardStates[i] = PRESSED; 
        else if (s_keyboardStates[i] == UP)
            s_keyboardStates[i] = NONE;   
    }

    for (int i = 0; i < Mouse::AMOUNT; i++)
    {
        if (s_mouseStates[i] == DOWN)
            s_mouseStates[i] = PRESSED;
        else if (s_mouseStates[i] == UP)
            s_mouseStates[i] = NONE;
    }

    for (int i = 0; i < Gamepad::AMOUNT; i++)
    {
        if (s_gamepadStates[i] == DOWN)
            s_gamepadStates[i] = PRESSED;
        else if (s_gamepadStates[i] == UP)
            s_gamepadStates[i] = NONE;
    }
}
